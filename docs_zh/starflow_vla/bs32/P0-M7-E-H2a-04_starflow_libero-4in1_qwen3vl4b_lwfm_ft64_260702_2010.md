...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **31187 / 80000**（38.98%） |
| **单步耗时** | ~9.17 s/it |
| **已运行时间** | 约 80 小时 10 分钟 |
| **预计剩余时间** | ~124 小时（约 5.2 天） |
| **最新 checkpoint** | `steps_31000` |
| **上一个 checkpoint** | `steps_30750` |
| **下一轮 eval** | `steps_31500`（~313 steps 后） |
...
| 31180 | 0.0678 | 0.0802 | |
| 31187 | — | — | 🔵 当前 |
| **250–31000** | — | — | ✅ 每 250 steps checkpoint |

### Loss 趋势

- 初始快速下降至 0.24（step 500），随后逐渐收敛
- 🔥🔥🔥 **最佳 eval mse 0.00346**（step 30000），step 31000 eval mse=**0.00438**（正常波动范围）
- step 13000–31100 损失在 0.030–0.12 之间震荡，训练 loss 新低 **0.0410** 在 step 30560
- 学习率按 cosine schedule 缓慢衰减（当前 base LR ~1.71e-5）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → 21500(0.00701) → 22000(0.00435) → 22500(0.00512) → 23000(0.00460) → 23500(0.00429) → 24000(0.00519) → 24500(0.00436) → 25000(0.00436) → 25500(0.00592) → 26000(0.00568) → 26500(0.00554) → 27000(0.00426) → 27500(0.00677) → 28000(0.00474) → 28500(0.00580) → 29000(0.00391) → 29500(0.00567) → 30000(**0.00346🏆**) → 30500(0.00380) → **31000(0.00438)**
- 最新 checkpoint **steps_31000** 已于 03:57 保存并同步至持久存储
...
├── checkpoints/steps_250/ ... steps_31000/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_31000`。
