...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **32282 / 80000**（40.35%） |
| **单步耗时** | ~9.20 s/it |
| **已运行时间** | 约 82 小时 59 分钟 |
| **预计剩余时间** | ~122 小时（约 5.1 天） |
| **最新 checkpoint** | `steps_32250` |
| **上一个 checkpoint** | `steps_32000` |
| **下一轮 eval** | `steps_32500`（~218 steps 后） |
...
| 32280 | 0.0612 | 0.0648 | |
| 32282 | — | — | 🔵 当前 |
| **250–32250** | — | — | ✅ 每 250 steps checkpoint |

### Loss 趋势

- 初始快速下降至 0.24（step 500），随后逐渐收敛
- 🔥🔥🔥 **最佳 eval mse 0.00346**（step 30000），step 32000 eval mse=**0.00439**（正常波动范围）
- step 13000–32280 损失在 0.030–0.12 之间震荡，训练 loss 新低 **0.0407** 在 step 31140
- 学习率按 cosine schedule 缓慢衰减（当前 base LR ~1.66e-5）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → 21500(0.00701) → 22000(0.00435) → 22500(0.00512) → 23000(0.00460) → 23500(0.00429) → 24000(0.00519) → 24500(0.00436) → 25000(0.00436) → 25500(0.00592) → 26000(0.00568) → 26500(0.00554) → 27000(0.00426) → 27500(0.00677) → 28000(0.00474) → 28500(0.00580) → 29000(0.00391) → 29500(0.00567) → 30000(**0.00346🏆**) → 30500(0.00380) → 31000(0.00438) → 31500(0.00385) → **32000(0.00439)**
- 最新 checkpoint **steps_32250** 已于 07:10 保存并同步至持久存储
...
├── checkpoints/steps_250/ ... steps_32250/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_32250`。
