...
> **最后训练日志**: unknown，checkpoint steps_9750
> **当前更新**: 2026-06-22 20:38:35 CST
> **tmux 会话**: 无（训练已停止）
...
| **已运行时间** | 30小时 17分钟（估计，至停止时） |
| **预计剩余时间** | N/A（已停止） |
| **预计总耗时** | N/A（已停止） |

> ⚠️ 实验已于 unknown 左右停止，最后 checkpoint 为 steps_9750。当前无运行中的训练进程。
---

## 系统资源占用

> 最后更新：2026-06-22 20:38:35 CST（训练已停止，显示当前系统快照）
...
| **已产生成本（至停止时）** | 30.3 h × 5.58 ≈ **169.02 元** |
...
- **当前状态**: 🔴 本实验已于 unknown 左右停止，最后 checkpoint 为 `steps_9750`（12.2%）。
- 实验曾多次 resume：最初停止在 step 260，resume 至 step 500 后再停止；6/21 从 steps_500 resume 继续训练，最终运行至 step ~9750。
...
| 字段 | 值 |
| --- | --- |
| 监控时间 | 2026-06-22 20:38:35 CST |
| 训练状态 | 🔴 已停止 |
| run_id | `P0-M6-H2-total-baseline_starflow_libero-4in1_qwen3vl4b_mlp_260621_1421` |
| tmux 会话 | 无（训练已停止） |
| 最终步数 | **9750 / 80000**（来自最新 checkpoint） |
| 完成比例 | 12.2% |
| 已运行时间 | 30小时 17分钟（估计） |
