...
> 最后更新：2026-07-05 04:30:00 CST

| 指标 | 值 |
|------|-----|
| **当前 Step** | **22166 / 80000**（27.71%） |
| **单步耗时** | ~9.14 s/it |
| **已运行时间** | 约 56 小时 59 分钟 |
| **预计剩余时间** | ~147 小时（约 6.1 天） |
| **最新 checkpoint** | `steps_22000` |
| **上一个 checkpoint** | `steps_21750` |
| **下一轮 eval** | `steps_22500`（~334 steps 后） |
...
| 22160 | 0.0578 | 0.0710 | |
| 22166 | — | — | 🔵 当前 |
| **250–22000** | — | — | ✅ 每 250 steps checkpoint |
...
- 🔥 **新的最佳 eval mse 0.00435** 在 step 22000，超越此前 best 0.00442（19000/19500，-1.6%）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → 21500(0.00701) → **22000(0.00435 🏆)**
- step 21000 的 spike（0.01054）已完全回归，模型收敛持续改善
- 最新 checkpoint **steps_22000** 已于 04:49 保存并同步至持久存储
...
├── checkpoints/steps_250/ ... steps_22000/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_22000`。
- 🔥🔥 **新最佳 eval mse 0.00435**（22000），超越此前 best 0.00442，模型收敛持续改善！
- 下一轮 eval 在 step 22500。
