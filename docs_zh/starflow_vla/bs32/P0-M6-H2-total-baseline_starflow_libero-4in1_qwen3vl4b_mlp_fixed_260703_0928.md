...
> 最后更新：2026-07-03 19:19:00 CST

| 指标 | 值 |
|------|-----|
| **当前 Step** | **9597 / 80000**（12.0%） |
| **完成比例** | 12.0% |
| **单步耗时** | ~3.56 s/it |
| **数据加载耗时** | ~0.000 s |
| **模型前向/反向耗时** | ~0.129 s |
| **已运行时间** | 约 9 小时 |
| **预计剩余时间** | ~70 小时（约 2.9 天） |
| **最新 checkpoint** | `steps_9500` |
...
- step 5000-8500：在 0.18-0.21 区间震荡，最低 0.180（step 8560），MLP 已基本收敛
...
| **显存空闲** | 14,234 MiB |
| **功耗** | 133.35 W / 450.00 W |
| **温度** | 47°C |

> MLP head 显存仅 ~10.3 GB，远低于 DiT 的 ~23.8 GB（P0-M7）。GPU 利用率偏低（~31%），per_device_batch_size=1 + grad_accum=32 导致 GPU 空闲较多。
...
| **Docker 内存已用** | ~20.4 GiB
| **Docker 内存可用** | ~35.6 GiB
| **Swap** | 0 B |
