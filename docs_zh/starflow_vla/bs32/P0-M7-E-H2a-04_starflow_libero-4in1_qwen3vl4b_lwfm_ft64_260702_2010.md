...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **24969 / 80000**（31.21%） |
| **单步耗时** | ~9.18 s/it |
| **已运行时间** | 约 64 小时 10 分钟 |
| **预计剩余时间** | ~140 小时（约 5.8 天） |
| **最新 checkpoint** | `steps_24750` |
| **上一个 checkpoint** | `steps_24500` |
| **下一轮 eval** | `steps_25000`（~31 steps 后）🔥 |
...
| 24960 | 0.0722 | 0.1007 | |
| 24969 | — | — | 🔵 当前 |
| **250–24750** | — | — | ✅ 每 250 steps checkpoint |

### Loss 趋势

- 初始快速下降至 0.24（step 500），随后逐渐收敛
- 🔥🔥 **最佳 eval mse 0.00429**（step 23500），step 24500 eval mse=**0.00436**（接近 best ✅）
- step 13000–24500 损失在 0.049–0.12 之间震荡，无明显上升
- 学习率按 cosine schedule 缓慢衰减（当前 base LR ~1.99e-5）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → 21500(0.00701) → 22000(0.00435) → 22500(0.00512) → 23000(0.00460) → 23500(**0.00429🏆**) → 24000(0.00519) → **24500(0.00436)**
- 最新 checkpoint **steps_24750** 已于 11:52 保存并同步至持久存储
