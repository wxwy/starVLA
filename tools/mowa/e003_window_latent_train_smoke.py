"""E-003 window-latent training smoke for MoWAFutureLatentPrior.

This script wires ``MoWAWindowLatentSampleDataset`` (episode-level latent cache +
window manifest) into a short training loop for the E-003 future-latent-prior
module.  It is intentionally standalone and does not touch
``starVLA/training/train_starvla.py``.

Language is represented with a small learnable embedding table.  This is a
smoke placeholder: a real run must replace it with the actual text encoder.
"""
...
def _collate_e003(batch: list[WindowLatentSample]) -> dict[str, Any]:
    return {
        "current_latent": torch.stack([sample.current_latent for sample in batch]),
        "future_latents": torch.stack([sample.future_latents for sample in batch]),
...
def _build_language_embedding(
    ...
    embedding = nn.Embedding(len(languages), text_hidden_dim)
    return embedding, lang_to_idx


def run_e003_window_latent_train_smoke(
    config: dict[str, Any],
    *,
    ...
    """Run the E-003 future-latent-prior training smoke.
    ...
    _validate_cache_latent_dim(
        cache_root,
        video_keys,
        config.get("interface", {}).get("current_latent_dim", 1024),
    )
    ...
    model_config = MoWAFutureLatentPriorConfig(
        current_latent_dim=interface.get("current_latent_dim", 1024),
        text_hidden_dim=interface.get("text_hidden_dim", 2048),
        hidden_dim=interface.get("hidden_dim", 2048),
        future_latent_dim=interface.get("future_latent_dim", 1024),
    )
    model = MoWAFutureLatentPrior(model_config).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)

    language_embedding, lang_to_idx = _build_language_embedding(
        dataset, model_config.text_hidden_dim
    )
    ...
    for batch_index, batch in enumerate(dataloader):
        ...
        text_hidden = language_embedding(indices)
        future_latent_target = future_latents.mean(dim=1)  # [B, D]

        loss, _metrics, output = model.compute_loss(
            current_latent,
            text_hidden,
        ...
        final_shapes = {
            "current_latent": list(current_latent.shape),
            "text_hidden": list(text_hidden.shape),
            "future_latent_target": list(future_latent_target.shape),
            "predicted_future_latent": list(output.predicted_future_latent.shape),
        }
    ...
    go_no_go = (
        "Go"
        if steps_completed == train_steps and loss_is_finite
        else "No-Go"
    )

    report = {
        "experiment_id": config.get("experiment_id", "E-003"),
        "stage": config.get("stage", "future_latent_prior"),
        ...
        "go_no_go": go_no_go,
        "notes": [
            "Language is represented by a learnable embedding table; replace with the real text encoder for production.",
            "Future latent target is the mean-pooled future window; this is a smoke simplification.",
        ],
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="E-003 future-latent-prior window-latent training smoke."
    )
    parser.add_argument(
        "--config-yaml",
        type=Path,
