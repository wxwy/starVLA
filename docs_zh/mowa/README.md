...
MoWA（Mobile World Action Model，面向移动操作的世界动作模型）是面向 long-horizon household mobile manipulation 的 World Action Model 工程启动包。MoWA 不替代 VLA 策略项目，不训练通用视频生成模型，而是围绕 action-relevant future representation 建立数据门禁、`full_heads`、`future_latent_prior`、`frozen_decoder_diagnostic` 等路线、实验 Registry、Agent 执行规则与日志追踪。

## 核心阅读顺序

1. `00_project_proposal.md`：理解项目目标、研究边界和各阶段路线的立项定位。
2. `01_technical_survey.md`：理解技术路线、数据候选和各阶段路线的调研依据与风险。
...
当前 checkout 中三份核心 Source-of-Truth 文件已恢复到仓库。后续执行必须直接以这三份文档为准；若未来再次缺失，只能在 readiness / implementation log 中记录缺口，不能本地补写替代版核心 SOT。
...
- 各阶段实验定义、实验编号、负面清单、future action 防泄漏、LayerwiseFM 默认 bridge/adapter 接入边界均以 `02_detailed_design.md` 为准。

## 执行门禁

不得直接跳过 G0 进入 `full_heads` / `future_latent_prior` 主训练。G0 未完成前，只允许生成数据门禁 skeleton、schema 草案、window sampler 设计、leakage test 计划和报告模板；不得启动这些主训练链路，任何 fps、Hz、window、batch、显存、训练时长或阈值都必须保持 `Data Gate` / `TBD` / `target` 口径。
