...
> 最后更新：2026-07-03 17:55:00 CST

| 指标 | 值 |
|------|-----|
| **当前 Step** | **8570 / 80000**（10.7%） |
| **完成比例** | 10.7% |
| **单步耗时** | ~3.61 s/it |
| **数据加载耗时** | ~0.000 s |
| **模型前向/反向耗时** | ~0.106 s |
...
| **显存空闲** | 14,234 MiB |
| **功耗** | 141.66 W / 450.00 W |
| **温度** | 51°C |

> MLP head 显存仅 ~10.3 GB，远低于 DiT 的 ~23.8 GB（P0-M7）。GPU 利用率偏低（~25%），per_device_batch_size=1 + grad_accum=32 导致 GPU 空闲较多。
