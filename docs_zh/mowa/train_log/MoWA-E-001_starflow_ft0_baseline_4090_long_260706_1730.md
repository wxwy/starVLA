...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **~64150 / 80000**（80.2%） |
| **完成比例** | 80.2% ✅ **突破 80%** 🏁 |
| **单步耗时** | ~3.82 s/it |
| **模型前向/反向耗时** | ~0.20–0.25 s |
| **数据加载耗时** | ~0.0002–0.0005 s（几乎忽略） |
| **已运行时间** | ~67 小时（约 2.8 天） |
| **预计剩余时间** | ~16.8 小时（约 **0.7 天**，**即将完成！** ）|
| **最新 checkpoint** | ✅ `steps_64000`（12:24 保存） |
| **最新 eval mse_score** | 始终未见 eval 输出 |

### Loss 记录

**完整记录共 1283 个数据点**（step 50–64150）。以下列出 milestone 及关键低点：

| Step | action_dit_loss | last_micro_loss | LR (action_model) | LR (base) | 备注 |
|------|----------------|-----------------|-------------------|-----------|------|
| 50 | 0.9581 | 0.7684 | 1.0e-5 | 2.5e-6 | warmup |
| **58550** | — | **0.0031** | 2.06e-5 | 5.16e-6 | 🏆 **最佳** |
| 62000 | — | — | — | — | ✅ checkpoint（10:17）|
| 62200 | — | **0.0121** | 1.57e-5 | 3.92e-6 | |
| 62650 | — | **0.0162** | 1.50e-5 | 3.74e-6 | |
| 63050 | — | **0.0071** | 1.44e-5 | 3.60e-6 | |
| 63150 | — | **0.0097** | 1.43e-5 | 3.57e-6 | |
| **63650** | — | **0.0057** | 1.37e-5 | 3.42e-6 | |
| 63950 | — | **0.0167** | 1.33e-5 | 3.33e-6 | |
| 64000 | — | **0.0684** | 1.33e-5 | — | ✅ checkpoint（12:24）|
| 64050 | — | **0.0184** | 1.32e-5 | 3.31e-6 | |
| 64150 | — | 0.2122 | 1.31e-5 | 3.28e-6 | 🔵 当前 |

### Loss 趋势

- 🏆 **最佳 last_micro_loss 保持 0.0031**（step 58550）。近期仍有多个 < 0.01 低值（63050: 0.0071, 63650: 0.0057）。
- ✅ **80% 完成**！训练进入最后冲刺（剩余 ~16.8h）。
- **Cosine LR decay 已近 87%**: action_model LR 1.0e-4 → **1.31e-5**（↓87%），base LR 2.5e-5 → **3.28e-6**（↓87%）。
- **Epoch**: 已完成 **0.58 epoch**（~58% 数据遍历）。
- 近期 loss 在 0.0057–0.21 区间，大部分稳定在 0.01–0.07。
- **GPU 利用率 75%**（运行以来最高）。
...
- 🟢 **训练正常运行中** — step ~64150 / 80000（**80.2% ✅**），已稳定运行 67h+。
- 🏆 **最佳 last_micro_loss: 0.0031**（step 58550）。近期仍有 < 0.01 低值（0.0057, 0.0071）。
- ✅ 最新 checkpoint `steps_64000`（12:24）。训练进入最后冲刺。
- 📉 **Cosine LR decay 近 87%**: action_model LR 1.0e-4 → **1.31e-5**（↓87%），base LR 2.5e-5 → **3.28e-6**（↓87%）。
- 🟢 **GPU 利用率 75%**（运行以来最高），显存稳定 82.7%。
- ⚠️ 全程未见 eval mse_score 输出。
- 预计完成时间：~16.8h（约 0.7 天），即 **7 月 10 日凌晨** 🏁。
