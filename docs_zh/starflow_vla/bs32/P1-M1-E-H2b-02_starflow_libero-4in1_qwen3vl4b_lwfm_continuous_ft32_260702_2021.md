...
> 最后更新：2026-07-07 10:31:00 CST

| 指标 | 值 |
|------|-----|
| **最终 Step** | **~56520 / 80000**（70.65%）⚠️ 训练中断 |
| **完成比例** | 70.65% |
| **单步耗时** | ~6.9-7.0 s/it |
| **数据加载耗时** | ~0.000 s |
| **模型前向/反向耗时** | ~0.21-0.22 s |
| **已运行时间** | 约 110 小时 |
| **最后一个 checkpoint** | `steps_56500`（已持久化） |
| **日志最后记录** | `step 56520` @ 10:16:39（进程退出） |
...
- 🔴 **训练中断**：step 56520 后训练进程退出，原因可能是 OOM 或环境重启。最终 loss 0.0271（action_dit_loss），last_micro_loss 0.0103。
- 已持久化的最新 checkpoint `steps_56500` 保留完整，可从此恢复训练。
...
| **GPU 利用率** | N/A（进程已退出） |
| **显存使用** | 已释放 |
| **训练中止原因** | step 56520 后无新日志，推测 OOM 或环境重启 |
