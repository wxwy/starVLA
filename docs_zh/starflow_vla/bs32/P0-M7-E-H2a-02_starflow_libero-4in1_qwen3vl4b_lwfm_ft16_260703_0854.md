...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **53380 / 80000**（66.7%） |
| **完成比例** | 66.7% |
| **单步耗时** | ~6.71 s/it |
| **模型前向/反向耗时** | ~0.21 s |
| **数据加载耗时** | ~0.000 s |
| **已运行时间** | 约 99.2 小时（4.1 天） |
| **预计剩余时间** | ~49 小时（约 2.1 天） |
| **最新 checkpoint** | `steps_53250` |
| **最新 eval mse_score** | **0.00735**（step 53000） |
| **上一个 eval mse_score** | 0.00871（step 52500） |
...
- eval mse_score 在 step 53000 为 **0.00735**，已大幅优于 H2a-04 最终水平（0.00549 step 5500，但 H2a-04 仅训练到 5500 步）
- grad_accum=32 导致 last_micro_loss 与平均 loss 的方差较大（单条样本 vs 32 条平均）
- 学习率已衰减：action_model ~2.94e-5，base ~7.35e-6
