...
> 最后更新：2026-07-03 16:26:00 CST

| 指标 | 值 |
|------|-----|
| **当前 Step** | **7373 / 80000**（9.2%） |
| **完成比例** | 9.2% |
| **单步耗时** | ~3.59 s/it |
| **数据加载耗时** | ~0.000 s |
| **模型前向/反向耗时** | ~0.110 s |
| **已运行时间** | 约 6 小时 52 分钟 |
| **预计剩余时间** | ~72 小时（约 3.0 天） |
| **最新 checkpoint** | `steps_7250` |
...
| 960 | 0.2532 | |
| 980 | 0.2518 | |
| 1000 | 0.2277 | checkpoint |
| 2000 | 0.2130 | |
| 3000 | 0.2166 | |
| 4000 | 0.2008 | |
| 5000 | 0.1997 | |
| 6000 | 0.1967 | checkpoint |
| 7000 | 0.1943 | |
| 7320 | 0.1943 | |
| 7340 | 0.1997 | |
| **7360** | **0.1859** | 🔵 当前 |
...
- MLP baseline loss 略高于 DiT（P0-M7 ft64: ~0.10–0.13 @ step ~1000），符合预期（MLP 容量远小于 DiT）
- 但单步耗时 ~3.5 s/it，远快于 DiT 的 ~9.2 s/it（P0-M7），3.2 天可跑完 80000 steps
...
| **显存空闲** | 14,234 MiB |
| **功耗** | 127.62 W / 450.00 W |
| **温度** | 50°C |

> MLP head 显存仅 ~10.3 GB，远低于 DiT 的 ~23.8 GB（P0-M7）。GPU 利用率偏低（~26%），per_device_batch_size=1 + grad_accum=32 导致 GPU 空闲较多。
...
| **Docker 内存已用** | ~21.3 GiB
| **Docker 内存可用** | ~34.7 GiB
| **Swap** | 0 B |
