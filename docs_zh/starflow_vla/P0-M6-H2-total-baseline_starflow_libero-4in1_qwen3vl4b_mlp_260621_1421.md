...
> **状态**: 🔴 已停止（checkpoint 目录缺失）
> **启动时间**: 2026-06-21 14:21:08 CST  
> **最后 checkpoint**: 2026-06-22 09:51:24 CST (`steps_9750`)  
> **最后训练日志**: unknown，checkpoint N/A
> **当前更新**: 2026-06-30 21:51:01 CST
> **tmux 会话**: 无（训练已停止）
...
> ⚠️ 本实验已停止。checkpoint 目录已不存在或为空，无法定位最后 checkpoint。 当前无运行中的训练进程。
---

## 系统资源占用

> 最后更新：2026-06-30 21:51:01 CST（训练已停止，显示当前系统快照）
...
| **已产生成本（至停止时）** | 0.0 h × 5.58 ≈ **0.00 元** |
| **完整 80000 steps 预估** | 80000 × 7.11s ≈ 158.0h ≈ **881.64 元**（未完成） |

> ⚠️ 训练已停止，成本不再增长。当前 GPU 空闲。
---
...
- **当前状态**: 🔴 本实验已停止。checkpoint 目录已不存在或为空，无法定位最后 checkpoint。
- 实验曾多次 resume：最初停止在 step 260，resume 至 step 500 后再停止；6/21 从 steps_500 resume 继续训练。
- 当前 checkpoint 目录 `/disk/rl/starVLA/playground/Checkpoints/P0-M6-H2-total-baseline_starflow_libero-4in1_qwen3vl4b_mlp_260621_1421/checkpoints` 已不存在或为空，无法继续 resume。
- 系统资源与成本数据反映当前系统快照（训练停止后）。
---
