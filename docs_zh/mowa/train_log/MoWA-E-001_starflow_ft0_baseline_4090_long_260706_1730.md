...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **~44900 / 80000**（56.1%） |
| **完成比例** | 56.1% |
| **单步耗时** | ~3.82 s/it |
| **模型前向/反向耗时** | ~0.20–0.25 s |
| **数据加载耗时** | ~0.0002–0.0005 s（几乎忽略） |
| **已运行时间** | ~47 小时 |
| **预计剩余时间** | ~37.3 小时（约 **1.6 天**，7/10 午前） |
| **最新 checkpoint** | ✅ `steps_44000`（15:39 保存） |
| **最新 eval mse_score** | 始终未见 eval 输出 |

### Loss 记录

**完整记录共 898 个数据点**（step 50–44900）。以下列出 milestone 及关键低点：
...
| **39400** | — | **0.0036** | 5.39e-5 | 1.35e-5 | 🏆 **最佳** |
| 42000 | — | — | — | — | ✅ checkpoint（13:35）|
| 42400 | — | **0.0091** | 4.75e-5 | 1.19e-5 | |
| 42800 | — | **0.0084** | 4.66e-5 | 1.17e-5 | |
| 43000 | — | **0.0125** | 4.62e-5 | 1.16e-5 | |
| 43550 | — | **0.0158** | 4.50e-5 | 1.12e-5 | |
| 43600 | — | **0.0110** | 4.48e-5 | 1.12e-5 | |
| 44000 | — | — | — | — | ✅ checkpoint（15:39）|
| 44050 | — | **0.0120** | 4.47e-5 | 1.12e-5 | |
| **44650** | — | **0.0089** | 4.37e-5 | 1.09e-5 | |
| 44750 | — | **0.0198** | 4.35e-5 | 1.09e-5 | |
| 44900 | — | 0.0471 | 4.32e-5 | 1.08e-5 | 🔵 当前 |

### Loss 趋势

- 🏆 **最佳 last_micro_loss 保持 0.0036**（step 39400）。近期持续出现 < 0.01 的低值（step 42400: 0.0091, step 42800: 0.0084, step 44650: 0.0089），**0.0036 记录有机会再次刷新**。
- **Cosine LR decay**: action_model LR 1.0e-4 → **4.32e-5**（↓57%），base LR 2.5e-5 → **1.08e-5**（↓57%）。
- **Epoch**: 已完成 **0.41 epoch**（~41% 数据遍历）。
- 近期 loss 在 0.0084–0.24 区间，大部分稳定在 0.01–0.09。Spike 幅度逐渐减小。
- GPU 利用率测到 **70%**，为运行以来最高值。
...
- 🟢 **训练正常运行中** — step ~44900 / 80000（**56.1%**），已稳定运行 47h+。
- 🏆 **最佳 last_micro_loss: 0.0036**（step 39400）。近期持续有 < 0.01 低值，记录仍可能刷新。
- ✅ 最新 checkpoint `steps_44000`（15:39）。已保存 14 个 checkpoint。
- 📉 **Cosine LR decay 过半**: action_model LR 1.0e-4 → **4.32e-5**（↓57%），base LR 2.5e-5 → **1.08e-5**（↓57%）。
- 🔴 **GPU 利用率 70%**（运行以来最高），显存稳定 82.7%。
- ⚠️ 全程未见 eval mse_score 输出。
- 预计完成时间：~37.3h（约 1.6 天），即 **7 月 10 日午前**。
