...
当前 checkout 中 `00_project_proposal.md`、`01_technical_survey.md`、`02_detailed_design.md` 未随仓库提供时，Agent 必须把该缺口视为 readiness 风险：不得临时补写或局部改写核心 SOT，只能读取 `03` 至 `10` 的派生执行文档、记录未解决项，并保持正式训练 launch gate 关闭。
...
4. 不得把 P2 扩展为 continuous video generation。
...
7. 不得新增未冻结 P0 head，不得做 per-head / leave-one-out / selected-head sweep。
...
2. G0/P0/P1/P2 相关实现至少包含 smoke test 或单元测试计划。
...
1. 必须先执行 M1 / G0 Data Verification Gate，再进入 P0/P1 主训练实现。
2. G0 不通过时不得强行启动 P0/P1；只能缩小数据域、补 schema 或输出降级报告。
3. P0/P1/P2 按 `03_agent_implementation_plan.md` 与 `05_experiment_registry.md` 推进。
