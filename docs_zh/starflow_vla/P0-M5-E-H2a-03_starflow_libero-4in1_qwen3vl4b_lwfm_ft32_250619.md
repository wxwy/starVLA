...
## 最新监控状态（2026-06-21 18:42 CST）

| 字段 | 值 |
| --- | --- |
| 监控时间 | 2026-06-21 18:42 CST |
| 训练状态 | ✅ 正常运行中 |
| 当前步数 | **13856 / 80000**（17%） |
| 最新完整 checkpoint | `steps_13875`（普通格式） |
| 最新 Loss | `action_dit_loss: 0.1304`（Step 13840） |
| 设备 | `NVIDIA A100-SXM4-80GB`（80 GB 显存） |
| 计费 | 5.58 元/h |
| GPU 利用率 | 100% |
| GPU 显存 | 63,473 MiB / 81,920 MiB（77.5%） |
| GPU 功耗 | 329.35 W / 400 W |
| GPU 温度 | 59°C |
| 训练速度 | 约 7.34 s/it |
| 已运行时间 | 5 小时 25 分钟 |
| 已产生成本 | **约 30.2 元** |
| 备注 | 当前 session 为 `P0-M5-E-H2a-03...250619`，`is_resume=True`；训练稳定推进 |
