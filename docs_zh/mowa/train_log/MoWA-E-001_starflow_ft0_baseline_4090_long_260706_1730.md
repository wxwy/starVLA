...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **~41050 / 80000**（51.3%） |
| **完成比例** | 51.3% ✅ **过半** |
| **单步耗时** | ~3.82 s/it |
| **模型前向/反向耗时** | ~0.20–0.25 s |
| **数据加载耗时** | ~0.0002–0.0005 s（几乎忽略） |
| **已运行时间** | ~43 小时 |
| **预计剩余时间** | ~41.3 小时（约 **1.7 天**，7/10 午前） |
| **最新 checkpoint** | ✅ `steps_40000`（11:31 保存） |
| **最新 eval mse_score** | 始终未见 eval 输出 |

### Loss 记录

**完整记录共 821 个数据点**（step 50–41050）。以下列出 milestone 及关键低点：
...
| **35900** | — | **0.0039** | 6.10e-5 | 1.53e-5 | |
| 37300 | — | **0.0057** | 5.75e-5 | 1.44e-5 | |
| 38000 | — | — | — | — | ✅ checkpoint（09:28）|
| 38050 | — | **0.0079** | 5.65e-5 | 1.41e-5 | |
| 38650 | — | **0.0131** | 5.52e-5 | 1.38e-5 | |
| 38950 | — | **0.0086** | 5.46e-5 | 1.37e-5 | |
| **39400** | — | **0.0036** | 5.39e-5 | 1.35e-5 | 🏆 **新最佳！** |
| 40000 | — | — | — | — | ✅ checkpoint（11:31）|
| 40050 | — | **0.0177** | 5.22e-5 | 1.31e-5 | |
| 40600 | — | **0.0177** | 5.13e-5 | 1.28e-5 | |
| 40850 | — | **0.0112** | 5.09e-5 | 1.27e-5 | |
| 41050 | — | 0.1627 | 5.05e-5 | 1.26e-5 | 🔵 当前 |

### Loss 趋势

- 🏆🔥 **最佳 last_micro_loss 再次刷新至 0.0036**（step 39400），相比 4 小时前的 0.0039 再降 7.7%！
- 已过半程（51.3%），模型持续改善中。step 37300 的 0.0057 和 step 38950 的 0.0086 也表现优异。
- **Cosine LR decay**: action_model LR 1.0e-4 → **5.05e-5**（↓49.5%），base LR 2.5e-5 → **1.26e-5**（↓49.6%）。
- **Epoch**: 已完成 **0.37 epoch**（~37% 数据遍历）。
- 近期 loss 在 0.0036–0.30 区间，大部分稳定在 0.01–0.15。偶发 spike（如 step 41000 的 0.30）属正常。
...
- 🟢 **训练正常运行中** — step ~41050 / 80000（**51.3% ✅ 已过半**），已稳定运行 43h+。
- 🏆🔥 **最佳 last_micro_loss 再次刷新至 0.0036**（step 39400），相比 4 小时前的 0.0039 再降 7.7%！模型仍在持续改善中。
- ✅ 最新 checkpoint `steps_40000`（11:31）。已保存 12 个 checkpoint。
- 📉 **Cosine LR decay 近半**: action_model LR 1.0e-4 → **5.05e-5**（↓49.5%），base LR 2.5e-5 → **1.26e-5**（↓49.6%）。
- 🔴 GPU 显存稳定 82.7%。
- ⚠️ 全程未见 eval mse_score 输出。
- 预计完成时间：~41.3h（约 1.7 天），即 **7 月 10 日午前**。
