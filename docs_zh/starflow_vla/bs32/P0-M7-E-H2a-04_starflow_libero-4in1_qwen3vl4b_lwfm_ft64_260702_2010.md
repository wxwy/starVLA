...
> 最后更新：2026-07-05 05:30:00 CST

| 指标 | 值 |
|------|-----|
| **当前 Step** | **22554 / 80000**（28.19%） |
| **单步耗时** | ~9.17 s/it |
| **已运行时间** | 约 57 小时 59 分钟 |
| **预计剩余时间** | ~146 小时（约 6.1 天） |
| **最新 checkpoint** | `steps_22500` |
| **上一个 checkpoint** | `steps_22250` |
| **下一轮 eval** | `steps_23000`（~446 steps 后） |
...
| 22540 | 0.0676 | 0.0714 | |
| 22554 | — | — | 🔵 当前 |
| **250–22500** | — | — | ✅ 每 250 steps checkpoint |
...
- 🔥 **最佳 eval mse 0.00435**（step 22000），step 22500 eval mse=**0.00512**（略回升，但优于之前多数 eval）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → 21500(0.00701) → 22000(**0.00435🏆**) → **22500(0.00512)**
- 最新 checkpoint **steps_22500** 已于 06:06 保存并同步至持久存储
...
├── checkpoints/steps_250/ ... steps_22500/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_22500`。
- 🔥 **最佳 eval mse 0.00435**（22000），step 22500 eval mse=**0.00512**（正常波动范围）
- 下一轮 eval 在 step 23000。
