...
## 最新监控状态（2026-06-22 19:09 CST）

| 字段 | 值 |
| --- | --- |
| 监控时间 | 2026-06-22 19:09 CST |
| 训练状态 | ✅ 正常运行中 |
| 当前步数 | **25730 / 80000**（32%） |
| 最新完整 checkpoint | `steps_25500`（普通格式，fused=True） |
| 最新 Loss | `action_dit_loss: 0.0922`（Step 25720） |
| 设备 | `NVIDIA A100-SXM4-80GB`（80 GB 显存） |
| 计费 | 5.58 元/h |
| GPU 利用率 | 0%（瞬时采样，训练在推进） |
| GPU 显存 | 63,473 MiB / 81,920 MiB（77.5%） |
| GPU 功耗 | 208.03 W / 400 W |
| GPU 温度 | 54°C |
| 训练速度 | 约 7.30–7.63 s/it |
| 已运行时间 | 约 22 小时 21 分钟（自 6/21 resume） |
| 累计费用 | 约 124.75 元 |
| 备注 | overnight 稳定推进至 32%，Loss 正常波动（0.05–0.15），checkpoint 每 250 步正常保存 |
...
| 2026-06-21 20:58 | 14835 | `steps_14875` | 用修正后的代码从 `steps_14750` resume 成功，fused AdamW 生效，Loss 约 0.075–0.085 |
| 2026-06-22 19:09 | 25730 | `steps_25500` | overnight 稳定推进至 32%，已运行 ~22.4h，累计费用约 124.75 元；Loss 正常波动 0.05–0.15，含 mse_score；GPU 瞬时利用率 0%（采样间隙），功耗 208W，温度 54°C |
