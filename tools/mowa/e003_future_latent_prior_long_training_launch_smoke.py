"""Run the E-003 formal long-training launch smoke."""
...
try:
    ...
    if exc.name != "tools":
        raise
    from e003_future_latent_prior_long_training_config_preview import (  # type: ignore[no-redef]
        build_e003_future_latent_prior_long_training_config_preview,
    )

CONFIG = Path("configs/mowa/mowa_e003_future_latent_prior_long_training_launch_candidate.yaml")
COMMAND_CONFIG = Path("configs/mowa/mowa_e003_training_command_long_training_candidate.yaml")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the MoWA E-003 long-training launch smoke.")
...
def main() -> None:
    args = parse_args()
    payload = build_e003_future_latent_prior_long_training_launch_smoke(
        args.repo_root,
        args.config_yaml,
...
def build_e003_future_latent_prior_long_training_launch_smoke(
    repo_root: Path | str,
    ...
    preview = build_e003_future_latent_prior_long_training_config_preview(root, config_yaml)
    ...
    checks = {
        ...
        "future_latent_prior_integrated_in_training_framework": _tree_contains(
            root,
            ("starVLA/training", "starVLA/model/framework"),
            "MoWAFutureLatentPrior",
        ),
        "latent_batch_contract_integrated_in_runtime_paths": _tree_contains_all(
            root,
            ("starVLA/training", "starVLA/model/framework", "starVLA/dataloader/gr00t_lerobot"),
            ("mowa_current_latent", "mowa_future_latent_target"),
        ),
        "future_latent_loss_integrated_in_training_loop": _tree_contains(
            root,
            ("starVLA/training", "starVLA/model/framework"),
            "future_latent_mse",
        ),
    ...
    return {
        ...
        "unresolved_items": [
            "Training framework wiring is present but a real-scale latent cache is still needed.",
...
def _tree_contains_all(root: Path, relative_dirs: Iterable[str], patterns: Iterable[str]) -> bool:
    return all(_tree_contains(root, relative_dirs, pattern) for pattern in patterns)
