...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **19828 / 80000**（24.79%） |
| **单步耗时** | ~9.15 s/it |
| **已运行时间** | 约 50 小时 59 分钟 |
| **预计剩余时间** | ~153 小时（约 6.4 天） |
| **最新 checkpoint** | `steps_19750` |
| **上一个 checkpoint** | `steps_19500` |
| **下一轮 eval** | `steps_20000`（~172 steps 后） |
...
| 19820 | 0.0775 | 0.1067 | |
| 19828 | — | — | 🔵 当前 |
| **250–19750** | — | — | ✅ 每 250 steps checkpoint |

### Loss 趋势

- 初始快速下降至 0.24（step 500），随后逐渐收敛
- 🔥 **最佳 eval mse 0.00442**（step 19000 和 step 19500 两次一致），模型已稳定收敛至该水平
- step 13000–19500 损失在 0.056–0.12 之间震荡，无明显上升
- 学习率按 cosine schedule 缓慢衰减（当前 base LR ~2.17e-5）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → **19500(0.00442)** —— 验证收敛稳定 🎯
- 最新 checkpoint **steps_19750** 已于 23:02 保存并同步至持久存储
...
├── checkpoints/steps_250/ ... steps_19750/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_19750`。
- 🔥 step 19000 和 step 19500 两次 eval mse 均为 **0.00442**，验证收敛稳定 🎯
- 下一轮 eval 在 step 20000。
