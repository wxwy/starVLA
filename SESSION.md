...
- E-001 launch / throughput 草案已补齐：
  ...
  - `tools/mowa/download_robocasa_remaining.py` 已作为可选数据维护工具纳入：默认 plan-only，不执行外网下载；必须显式传 `--execute` 才会下载/解压 target/composite 剩余任务。该工具不属于当前 target/human/atomic 主 recipe 的必要路径。
...
## 下一步
- 若继续推进 P0，runtime policy 的 checkpoint/save/resume 与离线 logging 已完成 prelaunch 确认，P0FullHeads bridge feature source 已完成 forward 接入，bounded full-VLA bs4 runtime smoke 已通过；还需要人工确认长训资源预算、创建可执行训练命令，并完成真实 action-gain / E-006 rollout 证据后才能把 `policy_confirmed` / `launch_ready` 置为 true。
