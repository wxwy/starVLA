...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-07-08 10:47 | 65300, | 0.03403667402744759 | 2.9445169744029894e-06 | 33% | 23800/24564 MiB | 61°C | —/56 GiB |
...
| 时间 | Step | Loss | LR | GPU util | GPU mem | GPU temp | Docker mem |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-07-08 04:47 | 62060, | 0.022842990529170493 | 3.856791047878737e-06 | 69% | 23800/24564 MiB | 64°C | —/56 GiB |

> 最后更新：2026-07-08 00:40:00 CST

| 指标 | 值 |
|------|-----|
| **当前 Step** | **59818 / 80000**（74.8%） |
| **完成比例** | 74.7% |
| **单步耗时** | ~6.63 s/it |
| **模型前向/反向耗时** | ~0.21 s |
| **数据加载耗时** | ~0.000 s |
| **已运行时间** | 约 111 小时（4.6 天） |
| **预计剩余时间** | ~37 小时（约 1.5 天） |
| **预计完成** | 2026-07-09 下午 |
| **最新 checkpoint** | `steps_59750` |
| **最新 eval mse_score** | **0.00769**（step 59500） |
| **最佳 eval mse_score** | **0.00660**（step 59000） |
| **上一个 eval mse_score** | 0.00660（step 59000） |
...
| **17640** | **0.0773** | |
| **18180** | **0.0708** | |
| **51500** | **0.0278** | eval mse_score=**0.00978** |
| **52000** | **0.0322** | eval mse_score=**0.00803** |
| **52500** | **0.0269** | eval mse_score=**0.00871** |
| **53000** | **0.0368** | eval mse_score=**0.00735**；✅ checkpoint |
| **53260** | **0.0327** | |
| **53340** | **0.0325** | |
| **53380** | **0.0275** | |
| **58000** | **0.0293** | eval mse_score=**0.00854** |
| **58500** | **0.0233** | eval mse_score=**0.00757** |
| **59000** | **0.0297** | eval mse_score=**0.00660** 🏆 最佳 |
| **59500** | **0.0272** | eval mse_score=**0.00769**；✅ checkpoint |
| **59620** | **0.0264** | |
| **59700** | **0.0228** | |
| **59782** | **0.0261** |
| **59818** | **0.0292** | 🔵 当前 |
...
- **Phase 2（140–5000）**: 震荡下降 0.38 → 0.13，学习逐步收敛
- **Phase 3（5000–53000）**: 稳步下降 0.13 → 0.032，eval mse_score 从 0.035 → 0.00735
- **Phase 4（53000–60000）**: 持续收敛 0.032 → 0.023-0.030 区间，eval mse_score 在 0.0066-0.0085 间波动
- 最佳 eval mse_score **0.00660** 出现在 step 59000，与 H2a-04 最终 mse 0.00549（step 5500）差距缩小
- grad_accum=32 导致 last_micro_loss 与平均 loss 的方差较大（单条样本 vs 32 条平均）
- 学习率已大幅衰减：action_model ~1.84e-5，base ~4.59e-6（余弦衰减 ~80% 周期）
- GPU 利用率在 30%-77% 间波动（per_device_batch=1, grad_accum=32 同步开销不稳定）
...
| **每 step 耗时** | ~6.6 s/it |
| **已运行时间** | ~111 小时（4.6 天） |
| **完整 80000 steps 预估** | 80000 × 6.6s ≈ 147h ≈ 6.1 天 |
| **预计完成时间** | 约 2026-07-09 下午 |
