...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **28853 / 80000**（36.07%） |
| **单步耗时** | ~9.19 s/it |
| **已运行时间** | 约 74 小时 10 分钟 |
| **预计剩余时间** | ~131 小时（约 5.5 天） |
| **最新 checkpoint** | `steps_28750` |
| **上一个 checkpoint** | `steps_28500` |
| **下一轮 eval** | `steps_29000`（~147 steps 后） |
...
| 28840 | 0.0526 | 0.0676 | |
| 28853 | — | — | 🔵 当前 |
| **250–28750** | — | — | ✅ 每 250 steps checkpoint |

### Loss 趋势

- 初始快速下降至 0.24（step 500），随后逐渐收敛
- 🔥🔥 **最佳 eval mse 0.00426**（step 27000），step 28500 eval mse=**0.00580**（正常波动范围）
- step 13000–28760 损失在 0.030–0.12 之间震荡，无明显上升
- 学习率按 cosine schedule 缓慢衰减（当前 base LR ~1.81e-5）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → 21500(0.00701) → 22000(0.00435) → 22500(0.00512) → 23000(0.00460) → 23500(0.00429) → 24000(0.00519) → 24500(0.00436) → 25000(0.00436) → 25500(0.00592) → 26000(0.00568) → 26500(0.00554) → 27000(**0.00426🏆**) → 27500(0.00677) → 28000(0.00474) → **28500(0.00580)**
- 最新 checkpoint **steps_28750** 已于 22:10 保存并同步至持久存储
...
├── checkpoints/steps_250/ ... steps_28750/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_28750`。
