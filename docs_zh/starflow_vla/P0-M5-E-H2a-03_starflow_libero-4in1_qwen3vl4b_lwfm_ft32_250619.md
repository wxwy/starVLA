...
## 最新监控状态（2026-06-21 19:42 CST）

| 字段 | 值 |
| --- | --- |
| 监控时间 | 2026-06-21 19:42 CST |
| 训练状态 | ✅ 正常运行中 |
| 当前步数 | **14336 / 80000**（18%） |
| 最新完整 checkpoint | `steps_14375`（普通格式） |
| 最新 Loss | `action_dit_loss: 0.1017`（Step 14320） |
| 设备 | `NVIDIA A100-SXM4-80GB`（80 GB 显存） |
| 计费 | 5.58 元/h |
| GPU 利用率 | 41%（本次采样偏低，训练仍在推进） |
| GPU 显存 | 63,473 MiB / 81,920 MiB（77.5%） |
| GPU 功耗 | 450.76 W / 400 W（读数高于 TDP，可能为瞬时峰值） |
| GPU 温度 | 59°C |
| 训练速度 | 约 7.31 s/it |
| 已运行时间 | 6 小时 25 分钟 |
| 已产生成本 | **约 35.8 元** |
| 备注 | 当前 session 为 `P0-M5-E-H2a-03...250619`，`is_resume=True`；GPU 利用率读数波动，功耗读数偏高 |
