...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **25283 / 80000**（31.60%） |
| **单步耗时** | ~9.17 s/it |
| **已运行时间** | 约 64 小时 59 分钟 |
| **预计剩余时间** | ~139 小时（约 5.8 天） |
| **最新 checkpoint** | `steps_25250` |
| **上一个 checkpoint** | `steps_25000` |
| **下一轮 eval** | `steps_25500`（~217 steps 后） |
...
| 25280 | 0.0672 | 0.0603 | |
| 25283 | — | — | 🔵 当前 |
| **250–25250** | — | — | ✅ 每 250 steps checkpoint |

### Loss 趋势

- 初始快速下降至 0.24（step 500），随后逐渐收敛
- 🔥🔥 **最佳 eval mse 0.00429**（step 23500），step 25000 eval mse=**0.00436**（连续两次 0.00436，验证稳定）
- step 13000–25280 损失在 0.049–0.12 之间震荡，无明显上升
- 学习率按 cosine schedule 缓慢衰减（当前 base LR ~1.96e-5）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → 21500(0.00701) → 22000(0.00435) → 22500(0.00512) → 23000(0.00460) → 23500(**0.00429🏆**) → 24000(0.00519) → 24500(0.00436) → **25000(0.00436)**
- 最新 checkpoint **steps_25250** 已于 13:09 保存并同步至持久存储
...
├── checkpoints/steps_250/ ... steps_25250/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_25250`。
- 🔥 **最佳 eval mse 0.00429**（23500），step 25000 eval mse=**0.00436**（稳定接近 best ✅）
- 下一轮 eval 在 step 25500。
