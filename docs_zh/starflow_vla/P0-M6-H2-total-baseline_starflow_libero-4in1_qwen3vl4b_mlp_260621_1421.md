...
> **当前更新**: 2026-06-21 15:24:46 CST  
> **tmux 会话**: `train`（当前该会话运行的是 `P0-M7-E-H2a-04`）  
...
- **当前状态**: tmux 会话 `train` 正在运行 `P0-M7-E-H2a-04`，本实验（P0-M6）已无活跃进程。最后训练日志为 step 260（loss 0.5204），最后完整 checkpoint 为 `steps_250`。
- 如需继续本实验，需从 `steps_250` resume 并重新占用 tmux 会话或新建会话。
- 系统资源与成本数据反映当前机器状态（由监控任务采集），GPU/显存占用主要来自当前运行的 `P0-M7-E-H2a-04`。
