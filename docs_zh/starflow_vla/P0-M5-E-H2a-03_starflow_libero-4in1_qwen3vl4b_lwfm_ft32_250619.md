...
## 最新监控状态（2026-06-22 19:12 CST）

| 字段 | 值 |
| --- | --- |
| 监控时间 | 2026-06-22 19:12 CST |
| 训练状态 | ✅ 正常运行中 |
| 当前步数 | **25757 / 80000**（32%） |
| 最新完整 checkpoint | `steps_25750`（普通格式，fused=True） |
| 最新 Loss | `action_dit_loss: 0.0723`（Step 25740） |
| 设备 | `NVIDIA A100-SXM4-80GB`（80 GB 显存） |
| 计费 | 5.58 元/h |
| GPU 利用率 | 94% |
| GPU 显存 | 63,473 MiB / 81,920 MiB（77.5%） |
| GPU 功耗 | 374.38 W / 400 W |
| GPU 温度 | 57°C |
| 训练速度 | 约 7.39–7.96 s/it |
| 已运行时间 | 约 22 小时 24 分钟（自 6/21 resume） |
| 累计费用 | 约 125.05 元 |
| 备注 | 稳定推进，Loss 正常波动（0.05–0.15），checkpoint 每 250 步正常保存 |
