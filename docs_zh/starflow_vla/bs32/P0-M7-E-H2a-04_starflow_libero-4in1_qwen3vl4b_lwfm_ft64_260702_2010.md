...
> 最后更新：2026-07-05 08:30:00 CST

| 指标 | 值 |
|------|-----|
| **当前 Step** | **24114 / 80000**（30.14%） |
| **单步耗时** | ~9.15 s/it |
| **已运行时间** | 约 61 小时 59 分钟 |
| **预计剩余时间** | ~142 小时（约 5.9 天） |
| **最新 checkpoint** | `steps_24000` |
| **上一个 checkpoint** | `steps_23750` |
| **下一轮 eval** | `steps_24500`（~386 steps 后） |
...
| 24100 | 0.0641 | 0.0751 | |
| 24114 | — | — | 🔵 当前 |
| **250–24000** | — | — | ✅ 每 250 steps checkpoint |

### Loss 趋势

- 初始快速下降至 0.24（step 500），随后逐渐收敛
- 🔥🔥 **最佳 eval mse 0.00429**（step 23500），step 24000 eval mse=**0.00519**（正常波动范围）
- step 13000–24100 损失在 0.049–0.12 之间震荡，无明显上升
- 学习率按 cosine schedule 缓慢衰减（当前 base LR ~2.00e-5）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → 21500(0.00701) → 22000(0.00435) → 22500(0.00512) → 23000(0.00460) → 23500(**0.00429🏆**) → **24000(0.00519)**
- 最新 checkpoint **steps_24000** 已于 09:57 保存并同步至持久存储
...
├── checkpoints/steps_250/ ... steps_24000/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_24000`。
- 🔥 **最佳 eval mse 0.00429**（23500），step 24000 eval mse=**0.00519**（正常波动）
- 下一轮 eval 在 step 24500。
