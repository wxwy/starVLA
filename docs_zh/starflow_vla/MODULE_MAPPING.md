...
| Base framework | `QwenPI_v3` | 计划继承或委托复用 |
| VLM backbone | Qwen3-VL via `get_vlm_model(config=...)` | 复用现有路径 |
| Projection layers | `QwenPI_v3.project_layers` | 复用现有路径 |
| Flow Matching action head | `LayerwiseFM_ActionHeader.py` | P0 默认路径 |
| H2 baseline | `MLP_ActionHeader.py`、`VLA_AdapterHeader.py`、`QwenOFT.py` | 待 P0-M6 |
| Action token / future token route | `future_tokens + cross-DiT` | 待 P0-M7 |
| State default path | QwenPI_v3 state-to-instruction | P0 默认路径 |
