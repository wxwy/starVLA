"""MoWA G0 RoboCasa365 read-only profile smoke check."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run MoWA G0 read-only profile smoke check.")
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[0, 1, 4],
        help="Episode indices to sample for timestamp/shape smoke.",
    )
    parser.add_argument("--preview-rows", type=int, default=32)
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
