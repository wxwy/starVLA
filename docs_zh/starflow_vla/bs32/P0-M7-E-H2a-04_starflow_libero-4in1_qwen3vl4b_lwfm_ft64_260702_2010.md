...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **25750 / 80000**（32.19%） |
| **单步耗时** | ~9.19 s/it |
| **已运行时间** | 约 66 小时 10 分钟 |
| **预计剩余时间** | ~138 小时（约 5.8 天） |
| **最新 checkpoint** | `steps_25750` |
| **上一个 checkpoint** | `steps_25500` |
| **下一轮 eval** | `steps_26000`（~250 steps 后） |
...
| 25740 | 0.0632 | 0.0617 | |
| 25750 | — | — | 🔵 当前 |
| **250–25750** | — | — | ✅ 每 250 steps checkpoint |

### Loss 趋势

- 初始快速下降至 0.24（step 500），随后逐渐收敛
- 🔥🔥 **最佳 eval mse 0.00429**（step 23500），step 25500 eval mse=**0.00592**（回升，类似 24000 模式）
- step 13000–25660 损失在 0.049–0.12 之间震荡，无明显上升
- 学习率按 cosine schedule 缓慢衰减（当前 base LR ~1.94e-5）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → 21500(0.00701) → 22000(0.00435) → 22500(0.00512) → 23000(0.00460) → 23500(**0.00429🏆**) → 24000(0.00519) → 24500(0.00436) → 25000(0.00436) → **25500(0.00592)**
- 最新 checkpoint **steps_25750** 已于 14:26 保存并同步至持久存储
...
├── checkpoints/steps_250/ ... steps_25750/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_25750`。
