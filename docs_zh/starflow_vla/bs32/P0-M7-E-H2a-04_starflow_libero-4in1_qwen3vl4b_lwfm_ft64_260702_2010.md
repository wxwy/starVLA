...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **23725 / 80000**（29.66%） |
| **单步耗时** | ~9.19 s/it |
| **已运行时间** | 约 60 小时 59 分钟 |
| **预计剩余时间** | ~144 小时（约 6.0 天） |
| **最新 checkpoint** | `steps_23500` |
| **上一个 checkpoint** | `steps_23250` |
| **下一轮 eval** | `steps_24000`（~275 steps 后） |
...
| 23720 | 0.0747 | 0.0843 | |
| 23725 | — | — | 🔵 当前 |
| **250–23500** | — | — | ✅ 每 250 steps checkpoint |

### Loss 趋势

- 初始快速下降至 0.24（step 500），随后逐渐收敛
- 🔥🔥 **新最佳 eval mse 0.00429**（step 23500），超越此前 best 0.00435（22000，-1.4%）
- step 13000–23700 损失在 0.056–0.12 之间震荡，无明显上升
- 学习率按 cosine schedule 缓慢衰减（当前 base LR ~2.02e-5）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → 21500(0.00701) → 22000(0.00435) → 22500(0.00512) → 23000(0.00460) → **23500(0.00429 🏆)**
- 最新 checkpoint **steps_23500** 已于 08:40 保存并同步至持久存储
...
├── checkpoints/steps_250/ ... steps_23500/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_23500`。
- 🔥🔥 **新最佳 eval mse 0.00429**（23500），超越此前 best 0.00435，模型收敛持续改善！
- 下一轮 eval 在 step 24000。
