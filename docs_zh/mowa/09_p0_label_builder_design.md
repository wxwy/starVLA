# MoWA Legacy Label Builder 设计索引

本文件为兼容留档入口，当前主路径请优先读取 `09_future_label_builder_design.md`。除兼容回查外，不再把本文件作为新的 readiness / launch / 实现主入口。

本文件是当前仓库内 legacy label coverage 与 label/mask builder 的派生设计索引，不是核心 Source-of-Truth。若与 `00_project_proposal.md`、`01_technical_survey.md`、`02_detailed_design.md` 冲突，以核心 SOT 为准；若未来核心 SOT 再次缺失，也不得在本文件中补写或改写其结论。
...
- future heads module: `starVLA/model/modules/mowa/p0_heads.py`
