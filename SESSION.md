...
- E-001 launch / throughput 草案已补齐：
  ...
  - Kimi 提到的 Major/Minor 尚未在本轮展开：P0 head 常量去重、ActionBridge 接口草案与候选配置数值统一、SOT 文档缺失处理、E-006 eval load run_id 参数化、anchor 选择一致性和 `action_outcome_class` 损失形式需要单独工程任务，避免混入 Critical 修复。
