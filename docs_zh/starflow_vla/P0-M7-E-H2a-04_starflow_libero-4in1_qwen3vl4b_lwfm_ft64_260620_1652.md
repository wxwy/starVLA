...
| **数据加载耗时** | 0.001 s |
| **模型前向/反向耗时** | 2.371 s |
| **已运行时间** | 0小时 20分钟（本次恢复后） |
| **预计剩余时间** | 6天 13小时 32分钟 |
| **预计总耗时** | 6天 13小时 52分钟（本次恢复后预估） |
...
| 指标 | 值 |
|------|-----|
| **CPU 使用率** | %Cpu(s):  2.1 us,  1.4 sy,  0.0 ni, 95.9 id,  0.0 wa,  0.0 hi,  0.5 si,  0.0 st |
| **内存总量** | 1007.5 GiB |
| **内存已用** | 87.0 GiB |
| **内存可用** | 860.0 GiB |
...
| 项目 | 计算 |
|------|------|
| **本次恢复后已产生成本** | 0小时 20分钟 × 5.58 元/h ≈ **1.86 元** |
| **按当前速度完整训练预估成本** | 6天 13小时 52分钟 × 5.58 元/h ≈ **878.75 元** |
| **剩余预计成本** | 6天 13小时 32分钟 × 5.58 元/h ≈ **876.89 元** |

> 注：本次为从 `steps_20000` 恢复训练，上表成本仅统计当前 tmux 会话（恢复后）的耗时。历史会话已产生的累计成本未计入。 |
...
- **2026-07-02 12:15:42 CST 更新**：训练从 `steps_20000` 恢复继续，当前 GPU 仍为 **NVIDIA A100-SXM4-80GB**（已与用户确认未切换至 4090）。有效 global batch size 保持 **32**（`per_device_batch_size=8` × `gradient_accumulation_steps=4`）。恢复后单步耗时约 **9.47 s/it**，WandB 提示存在部分历史 step 数据因恢复顺序问题被忽略。
...
| 已运行时间 | 0:20:08（本次恢复后） |
| 预计剩余时间 | 157:32:18 |
| 最新完整 checkpoint | `steps_20000`（已加载并恢复） |
