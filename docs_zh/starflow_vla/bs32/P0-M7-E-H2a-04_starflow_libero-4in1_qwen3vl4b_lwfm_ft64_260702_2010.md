...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **30333 / 80000**（37.92%） |
| **单步耗时** | ~9.17 s/it |
| **已运行时间** | 约 77 小时 59 分钟 |
| **预计剩余时间** | ~127 小时（约 5.3 天） |
| **最新 checkpoint** | `steps_30250` |
| **上一个 checkpoint** | `steps_30000` |
| **下一轮 eval** | `steps_30500`（~167 steps 后） |
...
| 30320 | **0.0490** | 0.0451 | |
| 30333 | — | — | 🔵 当前 |
| **250–30250** | — | — | ✅ 每 250 steps checkpoint |

### Loss 趋势

- 初始快速下降至 0.24（step 500），随后逐渐收敛
- 🔥🔥🔥 **新最佳 eval mse 0.00346**（step 30000），大幅超越此前 best 0.00391（-11.5%）🚀
- step 13000–30000 损失在 0.030–0.12 之间震荡，训练 loss 新低 0.0497
- 学习率按 cosine schedule 缓慢衰减（当前 base LR ~1.76e-5）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → 21500(0.00701) → 22000(0.00435) → 22500(0.00512) → 23000(0.00460) → 23500(0.00429) → 24000(0.00519) → 24500(0.00436) → 25000(0.00436) → 25500(0.00592) → 26000(0.00568) → 26500(0.00554) → 27000(0.00426) → 27500(0.00677) → 28000(0.00474) → 28500(0.00580) → 29000(0.00391) → 29500(0.00567) → **30000(0.00346 🏆)**
- 最新 checkpoint **steps_30250** 已于 02:02 保存并同步至持久存储
...
├── checkpoints/steps_250/ ... steps_30250/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_30250`。
