...
> 最后更新：2026-07-03 10:38:41 CST

| 指标 | 值 |
|------|-----|
| **当前 Step** | **6202 / 80000**（7.8%） |
| **完成比例** | 7.8% |
| **单步耗时** | ~3.34 s/it |
| **数据加载耗时** | ~0.000 s |
| **模型前向/反向耗时** | ~0.116 s |
| **已运行时间** | 约 58 分钟 |
| **预计剩余时间** | ~76 小时（约 3.2 天） |
| **最新 checkpoint** | `steps_6000` |
...
| **显存空闲** | 14,234 MiB |
| **功耗** | 140.38 W / 450.00 W |
| **温度** | 49°C |

> MLP head 显存仅 ~10.3 GB，远低于 DiT 的 ~23.8 GB（P0-M7）。GPU 利用率偏低（32%），可能由于 per_device_batch_size=1 导致 GPU 空闲较多。
...
| **Docker 内存已用** | ~21.5 GiB
| **Docker 内存可用** | ~34.5 GiB
| **Swap** | 0 B |
