...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **23019 / 80000**（28.77%） |
| **单步耗时** | ~9.18 s/it |
| **已运行时间** | 约 59 小时 10 分钟 |
| **预计剩余时间** | ~145 小时（约 6.0 天） |
| **最新 checkpoint** | `steps_23000` |
| **上一个 checkpoint** | `steps_22750` |
| **下一轮 eval** | `steps_23500`（~481 steps 后） |
...
| 23019 | — | — | 🔵 当前 |
| **250–23000** | — | — | ✅ 每 250 steps checkpoint |

### Loss 趋势

- 初始快速下降至 0.24（step 500），随后逐渐收敛
- 🔥 **最佳 eval mse 0.00442**（step 19000 和 step 19500），模型一度稳定收敛至该水平
...
- 🔥 **最佳 eval mse 0.00435**（step 22000），step 23000 eval mse=**0.00460**（接近 best，优于多数 eval）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → 21500(0.00701) → 22000(**0.00435🏆**) → 22500(0.00512) → **23000(0.00460)**
- 最新 checkpoint **steps_23000** 已于 07:23 保存并同步至持久存储

---

## 系统资源占用

> 最后更新：2026-07-05 07:30:00 CST
...
├── checkpoints/steps_250/ ... steps_23000/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_23000`。
- 🔥 **最佳 eval mse 0.00435**（22000），step 23000 eval mse=**0.00460**（接近 best ✅）
- 下一轮 eval 在 step 23500。
