...
## 最新监控状态（2026-06-22 20:37 CST）

| 字段 | 值 |
| --- | --- |
| 监控时间 | 2026-06-22 20:37 CST |
| 训练状态 | ✅ 正常运行中 |
| 当前步数 | **26446 / 80000**（33%） |
| 最新完整 checkpoint | `steps_26250`（普通格式，fused=True） |
| 最新 Loss | `action_dit_loss: 0.0820`（Step 26440） |
| 设备 | `NVIDIA A100-SXM4-80GB`（80 GB 显存） |
| 计费 | 5.58 元/h |
| GPU 利用率 | 100% |
| GPU 显存 | 63,473 MiB / 81,920 MiB（77.5%） |
| GPU 功耗 | 330.01 W / 400 W |
| GPU 温度 | 60°C |
| 训练速度 | 约 7.21–7.64 s/it |
| 已运行时间 | 约 23 小时 49 分钟（自 6/21 resume） |
| 累计费用 | 约 132.94 元 |
| 备注 | 稳定推进至 33%，epoch 0.52；Loss 正常波动，checkpoint 每 250 步正常保存 |
