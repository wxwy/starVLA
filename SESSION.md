...
- E-001 launch / throughput 草案已补齐：
  ...
  - `.venv/bin/python -m unittest tests.mowa.test_mowa_p0_heads -v` 最新已通过 14 项测试；`.venv/bin/python -m unittest tests.test_starflow_vla_reuse -v` 已通过 12 项测试，其中包含 dtype alignment opt-in 和 MoWA layerwise bridge coupling opt-in 边界测试。
