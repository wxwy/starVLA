...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **21775 / 80000**（27.22%） |
| **单步耗时** | ~9.20 s/it |
| **已运行时间** | 约 55 小时 59 分钟 |
| **预计剩余时间** | ~149 小时（约 6.2 天） |
| **最新 checkpoint** | `steps_21750` |
| **上一个 checkpoint** | `steps_21500` |
| **下一轮 eval** | `steps_22000`（~225 steps 后） |
...
| 21760 | 0.0624 | 0.0486 | |
| 21775 | — | — | 🔵 当前 |
| **250–21750** | — | — | ✅ 每 250 steps checkpoint |
...
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → **21500(0.00701)**
- step 21500 eval mse=**0.00701**，21000 的 spike（0.01054）已明显回落，验证为偶发异常
- 最新 checkpoint **steps_21750** 已于 04:11 保存并同步至持久存储
...
├── checkpoints/steps_250/ ... steps_21750/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_21750`。
- 🔥 最佳 eval mse **0.00442**（19000/19500），step 21500 eval mse=**0.00701**（21000 的 spike 已回落 ✅）
- 下一轮 eval 在 step 22000。
