...
| 指标 | 值 |
|------|-----|
| **当前 Step** | **26838 / 80000**（33.55%） |
| **单步耗时** | ~9.19 s/it |
| **已运行时间** | 约 68 小时 59 分钟 |
| **预计剩余时间** | ~136 小时（约 5.7 天） |
| **最新 checkpoint** | `steps_26750` |
| **上一个 checkpoint** | `steps_26500` |
| **下一轮 eval** | `steps_27000`（~162 steps 后） |
...
| 26820 | 0.0639 | 0.0478 | |
| 26838 | — | — | 🔵 当前 |
| **250–26750** | — | — | ✅ 每 250 steps checkpoint |

### Loss 趋势

- 初始快速下降至 0.24（step 500），随后逐渐收敛
- 🔥🔥 **最佳 eval mse 0.00429**（step 23500），step 26500 eval mse=**0.00554**（正常波动范围）
- step 13000–26500 损失在 0.049–0.12 之间震荡，无明显上升
- 学习率按 cosine schedule 缓慢衰减（当前 base LR ~1.91e-5）
- step 18000(0.00677) → 18500(0.00594) → 19000(0.00442) → 19500(0.00442) → 20000(0.00530) → 20500(0.00594) → 21000(0.01054 ⚠️) → 21500(0.00701) → 22000(0.00435) → 22500(0.00512) → 23000(0.00460) → 23500(**0.00429🏆**) → 24000(0.00519) → 24500(0.00436) → 25000(0.00436) → 25500(0.00592) → 26000(0.00568) → **26500(0.00554)**
- 最新 checkpoint **steps_26750** 已于 17:01 保存并同步至持久存储
...
├── checkpoints/steps_250/ ... steps_26750/  ✅
├── config.full.yaml          ✅
├── config.yaml               ✅
...
- checkpoint 每 250 steps 正常保存至 `steps_26750`。
