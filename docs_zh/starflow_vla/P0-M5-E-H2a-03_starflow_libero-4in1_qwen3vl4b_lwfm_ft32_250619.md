...
## 最新监控状态（2026-06-21 19:12 CST）

| 字段 | 值 |
| --- | --- |
| 监控时间 | 2026-06-21 19:12 CST |
| 训练状态 | ✅ 正常运行中 |
| 当前步数 | **14095 / 80000**（18%） |
| 最新完整 checkpoint | `steps_14125`（普通格式） |
| 最新 Loss | `action_dit_loss: 0.0843`（Step 14080） |
| 设备 | `NVIDIA A100-SXM4-80GB`（80 GB 显存） |
| 计费 | 5.58 元/h |
| GPU 利用率 | 59%（本次采样偏低，训练仍在推进） |
| GPU 显存 | 63,473 MiB / 81,920 MiB（77.5%） |
| GPU 功耗 | 363.38 W / 400 W |
| GPU 温度 | 59°C |
| 训练速度 | 约 7.56 s/it |
| 已运行时间 | 5 小时 55 分钟 |
| 已产生成本 | **约 33.0 元** |
| 备注 | 当前 session 为 `P0-M5-E-H2a-03...250619`，`is_resume=True`；GPU 利用率瞬时波动，功耗保持 363W |
